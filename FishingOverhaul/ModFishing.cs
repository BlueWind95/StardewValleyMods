    ...
    public class ModFishing : Mod {
        ...
        private void UpdateTick(object sender, EventArgs e) {
            ...
            if (Game1.player.CurrentTool is FishingRod) {
                ...
                if (bobber != null) {
                    if (_lastDurability.ContainsKey(bobber)) {
                        float last = _lastDurability[bobber];
                        bobber.scale.Y = last + (bobber.scale.Y - last) * Config.TackleDestroyRate;
                        if (bobber.scale.Y <= 0) {
                            _lastDurability.Remove(bobber);
                        ...
                        } else _lastDurability[bobber] = bobber.scale.Y;
                    } else _lastDurability[bobber] = bobber.scale.Y;
                }
            }
        }
        ...
        private void OnPostRenderHud(object sender, EventArgs e) {
            MineShaft mines = Game1.currentLocation as MineShaft;
            IEnumerable<KeyValuePair<int, ConfigFish.FishData>> possibleFish = FishHelper.GetPossibleFish(10, mines?.mineLevel ?? -1);
            foreach (KeyValuePair<int, ConfigFish.FishData> kv in possibleFish) {
                // Draw fish
